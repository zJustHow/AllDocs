...
_FORMAT_RULES = {
    "zh": (
        "格式：使用 Markdown，层级清晰、便于扫读。"
        "大节标题须用二级标题 `##`，格式为「## 1. 标题」「## 2. 标题」，独占一行；"
        "子节用三级标题 `###`（如「### 1.1 子标题」）。"
        "大节之间空一行。"
        "步骤与并列要点用 `-` 或有序列表，一项一行；"
        "勿在同一行用「–」或分号串联多项。"
        "关键术语、参数、按钮标识可用 **加粗**；安全警告可用引用块 `>`。"
        "引用 [n] 写在句末；下一条步骤或下一节须另起一行，"
        "勿紧接在同一行（错误示例：[1]。3.xxx）。"
        "段落之间空一行。"
    ),
    "en": (
        "Format: Use Markdown with clear visual hierarchy. "
        "Main sections must use `##` headings, e.g. `## 1. Title`, `## 2. Title`, "
        "each on its own line; use `###` for subsections (e.g. `### 1.1 Subtitle`). "
        "Leave a blank line between major sections. "
        "Use `-` or numbered lists for steps and bullet points—one item per line; "
        "do not join multiple items with dashes or semicolons on one line. "
        "Bold **key terms**, parameters, and button labels; use blockquotes `>` for safety warnings. "
...
def _build_system_prompt(lang: str) -> str:
    return "\n\n".join(
        [
            _CORE_RULES,
            _FORMAT_RULES.get(lang, _FORMAT_RULES["zh"]),
            _LANG_DIRECTIVE.get(lang, _LANG_DIRECTIVE["zh"]),
        ]
    )
