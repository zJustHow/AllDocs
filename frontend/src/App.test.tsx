...
const setDocumentChatEnabled = vi.fn();
...
vi.mock("./api", async () => {
  ...
  return {
    ...
    setDocumentChatEnabled: (...args: Parameters<typeof setDocumentChatEnabled>) =>
      setDocumentChatEnabled(...args),
...
function getRightPanelDomOrder(): Array<"viewer"> {
  const app = document.querySelector(".app");
  if (!app) return [];
  return Array.from(app.children)
    .filter((el) => el.classList.contains("doc-viewer-slot"))
    .map(() => "viewer" as const);
}
...
describe("App", () => {
  beforeEach(() => {
    ...
    setDocumentChatEnabled.mockImplementation(async (_id, chatEnabled) => ({
      ...sampleDocuments[0],
  ...
  it("links to the settings page from the header action", async () => {
    renderApp();

    await screen.findByText("Manual.pdf");
    const settingsLink = screen.getByRole("link", { name: /Open management|打开管理/i });
  ...
  it("does not reorder panels when updating an already open viewer", async () => {
    ...
    await user.click(await screen.findByRole("button", { name: "[1]" }));
    expect(getRightPanelDomOrder()).toEqual(["viewer"]);
  ...
  it("shows an error when sending without selected documents", async () => {
    ...
    expect(await screen.findByText(/Enable at least one ready|请至少启用/i)).toBeInTheDocument();
