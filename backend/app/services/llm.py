...
VISION_EXTRA_RULES = """
- 用户消息中附带了部分条目的页面图片，请读图理解表格、接线图和示意图后再作答
- 需要展示原文表格或图示时，由你根据读图结果决定在回答中的插入位置，使用 {{embed:N}}，N 为<context>中对应条目的编号（与 [N] 引用编号一致）
- 每张图片在回答中最多出现一次：不要重复插入相同 {{embed:N}}；若多处涉及同一图，仅在最合适的一处插入一次，其余用文字说明并保留 [N] 引用
- 表格类问题：优先在说明文字前用 {{embed:N}} 展示原表，再用文字解释要点
...
3. 参数规格 → search_chunks，filters.chunk_types 含 table（需 PDF 高亮标注）
4. snippet 不够 → read_chunks
