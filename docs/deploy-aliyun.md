...
本文基于仓库内现有生产配置（`docker-compose.prod.yml`、`scripts/server-bootstrap.sh`）整理，适用于在**单台阿里云 ECS** 上以 Docker Compose 运行完整 AllDocs 栈。

**代码同步方式（二选一）：**

1. **本地上传（默认文档流程）**：开发机 `rsync` 或压缩包上传至 `/opt/alldocs`，不依赖 GitHub 存放代码。
2. **GitHub Actions 自动部署（可选）**：推送 `main` 分支后，`.github/workflows/deploy.yml` 经 SSH `rsync` 同步代码并执行 `scripts/remote_compose_deploy.sh`。需在仓库 Secrets 配置 `ECS_HOST`、`ECS_USER`、`ECS_SSH_KEY`；`.env` 与 `models/` 仍只保留在服务器本地，不会被 CI 覆盖。
...
对外只开放 **前端 Nginx 的 80 端口**；API 通过 Nginx 的 `/api/` 和 WebSocket `/ws/` 转发，无需单独暴露 8000。Nginx 默认 `client_max_body_size 100m`（见 `frontend/nginx.conf`），大 PDF 上传需确保未在宿主机反向代理层进一步限缩。
...
docker compose -f docker-compose.prod.yml exec api \
  python -m app.cli create-admin --email admin@yourcompany.com --password your-strong-password
```
...
问答检索范围：所有 `ready` 且 `chat_enabled=true` 的文档（Admin 在侧边栏切换开关）。
...
- 日志：`docker compose -f docker-compose.prod.yml logs -f api worker-ingestion worker-maintenance`
- 指标：API `/metrics`、Inference `/metrics` 与 `/ready`、Worker `:9100`（内网，见 [observability.md](observability.md)）
...
## 十、GitHub Actions 自动部署（可选）

若 ECS 已按第三节完成首次 bootstrap（`.env`、`models/` 就绪），可启用 CI/CD：

1. 在 GitHub 仓库 **Settings → Secrets and variables → Actions** 配置：
   - `ECS_HOST`：ECS 公网 IP 或域名
   - `ECS_USER`：SSH 用户（如 `root`）
   - `ECS_SSH_KEY`：私钥全文（含 `BEGIN … PRIVATE KEY` 行）
2. 可选变量：`ECS_DEPLOY_PATH`（默认 `/opt/alldocs`）、`ECS_SSH_PORT`（默认 `22`）
3. 推送 `main` 或手动触发 **Deploy** workflow

Workflow 会先跑前后端测试，再 `rsync` 同步代码（排除 `.env`、`models/`），在服务器执行 `scripts/remote_compose_deploy.sh start` 并轮询健康检查。`.env` 变更后仍需在 ECS 上手动编辑并重启相关容器。
