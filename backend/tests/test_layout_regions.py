...
from app.services.ingestion_chunking import (
    block_spans_from_joined_blocks,
    concat_pages,
    regions_for_range,
...
class RegionsForRangeTests(unittest.TestCase):
    def test_single_page_uses_block_y_bounds(self) -> None:
        ...
        block_spans = block_spans_from_joined_blocks(
            ["Alpha", "Beta"],
            [(100.0, 180.0), (220.0, 300.0)],
        )

        regions = regions_for_range(page_spans, block_spans, 0, 5, total_len=10)
    ...
    def test_cross_page_chunk_returns_multiple_regions(self) -> None:
        ...
        section_text, page_spans, block_spans = concat_pages(pages)
        chunk_end = len(section_text)

        regions = regions_for_range(
            page_spans,
            block_spans,
            0,
            chunk_end,
    ...
    def test_partial_last_page_only_highlights_overlap(self) -> None:
        ...
        section_text, page_spans, block_spans = concat_pages(pages)
        chunks = split_text_with_offsets(section_text, chunk_size=999, overlap=0)
