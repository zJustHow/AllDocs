...
前后端共享契约位于 `backend/shared/`（`file_formats.json`、`markers.json`），由 `backend/app/services/shared_contract.py` 与前端 `@shared` 别名共同引用。

### 前端

Vite + React 单页应用，开发模式下由 Vite 代理 `/api`、`/ws` 至 `:8000`（见 `frontend/vite.config.ts`）。

| 路由 | 说明 |
|------|------|
| `/` | 主界面：文档侧边栏、对话、文档查看器、语音输入 |
| `/profile` | 账号面板：绑定 / 解绑邮箱、手机、微信 |
| `/settings` | 管理控制台（仅 Admin）：系统配置、用户管理、审计日志 |
| `/auth/callback` | 微信 OAuth 完成后写入 token |

登录页（`frontend/src/auth/LoginPage.tsx`）支持邮箱 OTP 注册、邮箱密码登录、手机 OTP、微信扫码。Admin 在侧边栏勾选各文档是否参与 RAG（`chat_enabled`），在设置页（`SettingsPage.tsx`）编辑可运行时覆盖的配置项。对话区展示 Agent 检索步骤（`AgentSteps.tsx`，对应 SSE `agent_*` 事件）。界面支持中英文切换（`frontend/src/i18n/`）。
...
```bash
./dev.sh --build       # 重新构建镜像
./dev.sh --docker      # 生产 Compose（Nginx 前端，端口 3000；同 --prod）
./dev.sh --stop        # 停止所有 Docker 服务
docker compose logs -f api worker-ingestion worker-maintenance
```

`dev.sh` 会启动 `api`、`worker-ingestion`、`worker-maintenance`、`postgres`、`redis`、`qdrant`、`minio` 等；`elasticsearch` 由 worker/api 的 `depends_on` 自动拉起。每次开发启动会重启 worker 容器以加载最新代码。
...
│   │   ├── db/               # SQLAlchemy 模型与会话
│   │   ├── services/         # ingestion, rag, llm, vector_store, shared_contract, …
│   │   │   └── agent/        # Retrieval Agent、answer_flow、tool_definitions、tools
│   │   ├── inference/        # 可选 BGE embed/rerank HTTP 服务
│   │   ├── workers/          # Celery 队列路由、入库/维护任务与请求上下文传递
│   │   ├── cli.py            # create-admin 等管理命令
│   │   └── observability.py  # JSON 日志、request/task 关联与 Prometheus 指标
│   ├── shared/               # 前后端共享契约（格式、标记正则）
│   └── tests/
├── frontend/src/             # Vite + React（auth/、i18n/、SettingsPage、AgentSteps 等）
├── docker/                   # Elasticsearch 等镜像构建
├── docs/
│   ├── deploy-aliyun.md          # 阿里云 ECS 部署
│   ├── observability.md          # 可观测性与 Prometheus
│   └── manual-writing-guide.md   # 操作手册编写规范（面向文档作者）
├── scripts/                  # bootstrap、模型下载、remote_compose_deploy 等
├── .github/workflows/        # CI 测试与可选 ECS 自动部署
├── dev.sh                    # 一键开发启动
├── docker-compose.yml
...
实现参考：`backend/app/services/ingestion.py`、`caption.py`、`pdf_captions.py`、`pdf_refs.py`、`pdf_attach_reading_order.py`、`asset_lookup.py`、`pdf_header_footer.py`、`pdf_tables.py`、`pdf_embedded_images.py`、`pdf_vlm_route.py`、`chunk_alignment.py`、`toc_lookup.py`、`fulltext_store.py`、`backend/app/services/agent/`、`backend/app/workers/tasks.py`。
...
Agent 限制（`.env`）：`RAG_AGENT_MAX_STEPS=10`、`RAG_AGENT_MAX_RETRIEVALS=6`、`RAG_BATCH_SEARCH_MAX=3`、`RAG_AGENT_MAX_PARALLEL_TOOLS=4`、`RAG_AGENT_KEEP_FULL_OBSERVATION_STEPS=1`、`RAG_AGENT_HISTORY_SNIPPET_MAX=60`、`RAG_AGENT_OUTLINE_PREVIEW_LINES=5`（默认，见 `config.py`）。
...
| `done` | 完成（含 `session_id`、`content`、`citations`、`embeds`、`language`） |
| `error` | 异常信息 |

Agent 内部 `ask_user` / 无证据 fallback 在 HTTP SSE 上映射为 `delta`（澄清或兜底文本）+ `done`（`citations`/`embeds` 为空，含 `language`），**不会**单独发送 `clarify` / `fallback` 事件类型。

语音通道 `WebSocket /ws/voice` 复用同一 Agent 流水线；事件含 `status`（`model_loading`、`transcribing`、`retrieving`、`answering`、`speaking`）、`answer_delta`、`audio` 等，回答经 Piper TTS 按句输出。
...
|------|-------|----------|
| 查看文档列表、预览、对话、语音 | ✅ | ✅ |
| 控制各文档是否参与 RAG（`chat_enabled`） | ✅ | ❌ |
| 上传 / 重新索引 / 删除文档 | ✅ | ❌ |
| 管理（系统配置、用户管理、审计日志） | ✅ | ❌ |
| 账号面板（绑定 / 解绑登录方式） | ✅ | ✅ |

问答检索范围：所有 `status=ready` 且 `chat_enabled=true` 的文档（Admin 与普通用户相同）。Admin 在侧边栏切换各文档的 `chat_enabled`（`PATCH /api/v1/documents/{id}/chat-enabled`）；`POST /chat` 请求体中的 `doc_ids` 当前不用于筛选。
...
```bash
docker compose exec api python -m app.cli create-admin --email admin@example.com --password your-secure-password
```
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/auth/register/email` | 邮箱注册（直接设密码；前端 UI 使用 OTP 注册流程） |
...
审计动作包括：`user.update`、`settings.update`、`document.upload`、`document.reindex`、`document.delete`、`document.chat_enabled`。
...
| GET | `/documents` | 文档列表（含 `chat_enabled`、`progress`、`ocr_pages` 等） |
| GET | `/documents/formats` | 支持格式 |
| GET | `/documents/{id}/file` | 下载原文件 |
| GET | `/documents/{id}/preview` | DOCX / HTML 安全预览 HTML |
| GET | `/documents/{id}/pages/{page}/render` | 页面 PNG（可选 `?scale=`，0.5–4.0，默认 2.0） |
| PATCH | `/documents/{id}/chat-enabled` | 切换是否参与 RAG（Admin） |
| POST | `/documents/{id}/reindex` | 重新入库 |
| DELETE | `/documents/{id}` | 异步删除（返回 `{"status": "deleting"}`） |
| POST | `/chat` | SSE 流式问答 |
| GET | `/assets/{asset_id}` | 表格/插图 PNG |
| GET | `/settings` | 可读运行时配置（Admin；含 `.env` 与数据库覆盖） |
| PATCH | `/settings` | 更新可编辑配置（Admin；持久化至 PostgreSQL，优先于 `.env`） |
...
| GET | `/admin/audit-logs` | 审计日志（Admin） |
| WS | `/ws/voice` | 语音问答 |
| GET | `/health` | 健康检查（`status`、`speech_ready`） |
...
部分配置可通过 Admin 设置页或 `PATCH /api/v1/settings` 运行时覆盖（见 `settings_registry.py`；不含 PDF/OCR/JWT/Worker 等项），优先级高于 `.env`。`CELERY_*` 仅由 Compose 读取，不在 `config.py` 中。
